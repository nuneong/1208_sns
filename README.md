...
- 환경별 Supabase 클라이언트 분리:
  ...
  - Server Component용 (`createClient` - Supabase 공식 문서 스타일)
...
3. **"Add provider"** 클릭
4. 제공자 목록에서 **"Clerk"** 선택
5. **3-1**에서 복사한 **Clerk domain**을 붙여넣기
6. **"Save"** 또는 **"Add Provider"** 클릭

> **참고**: 이 통합은 Clerk 세션 토큰에 `"role": "authenticated"` JWT claim을 자동으로 추가하여 Supabase API 요청이 인증된 사용자로 처리되도록 합니다.

**3-3. 통합 확인**

- [상세 통합 가이드](./docs/clerk-supabase-integration.md) 참고
...
2. 다음 값들을 복사하여 `.env` 파일에 입력:
   ...
   # 또는 NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY 사용 가능 (Supabase 공식 문서 스타일)
...
> **참고**: Supabase 공식 문서에서는 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`를 사용하지만, 이 프로젝트는 `NEXT_PUBLIC_SUPABASE_ANON_KEY`도 지원합니다. 둘 다 같은 값입니다.
...
자세한 내용은 [Clerk 로컬라이제이션 가이드](./docs/clerk-localization.md)를 참고하세요.
...
Supabase 공식 문서의 예제를 따라 `instruments` 테이블을 생성하고 데이터를 조회할 수 있습니다:

1. **테이블 생성**: Supabase Dashboard의 SQL Editor에서 다음 마이그레이션 실행:
   ```sql
   -- supabase/migrations/20250101000000_create_instruments_table.sql 파일 참고
   ```

2. **데이터 조회**: `/instruments` 페이지에서 테이블 데이터 확인
